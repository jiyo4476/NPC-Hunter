...
NPC-Hunter::buildMap(string filename)//~~~~~~UNFINISHED
{
...
void NPC-Hunter::addLocation(string name)
{
...
void NPC-Hunter::displayAvailablePaths()
{
    for(int i=0; i<locations.size(); i++) //steps through all locations
    {
...
void NPC-Hunter::displayNearbyNPCS()
{
...
void NPC-Hunter::displayPathLengths() //NOT FINISHED  Wondering if this should be absorbed by another method
{
...
void NPC-Hunter::movePlayer() //take user input to decide where to go // figure out how to set wait timer and time loop for that matter;
{
    ...
    if(choice!=NULL)
    {
        ...
        for(int i=0; i<locations.size();i++)
        {
            if(locations[i].name == choice)
            {
                ...
                u.name=User;
                u.waitTimer
                locations[i].atLocation.push_back("User")
